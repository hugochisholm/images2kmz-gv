...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--tui", action="store_true", help="Launch Textual User Interface"
    )
...
def prompt_for_directory(console: Console | None = None) -> str:
    ...
    while True:
        user_input = input(
            "Please provide a directory containing images to process: "
        ).strip()

        if not user_input:
            console.print(
                "[bold red]Error: Path cannot be empty. Please try again.[/bold red]"
            )
            continue
        ...
        if not Path(abs_path).is_dir():
            ...
            response = (
                input(f"Directory does not exist: {abs_path}. Create it? (y/n): ")
                .strip()
                .lower()
            )
...
def print_summary(
    ...
    if processed > 0 and kmz_generator and output_path:
        ...
        if file_size:
            console.print(
                f"[green]📦 Output: {Path(output_path).name} ({file_size})[/green]"
            )
        else:
...
def execute_run(parsed_args: argparse.Namespace, ui: UIHandler) -> int:
    ...
    try:
        ...
        ui.print_info(
            f"[cyan]Recursive search: {'Yes' if parsed_args.recursive else 'No'}[/cyan]"
        )
        ui.print_info(
            f"[cyan]Thumbnail size preset: {parsed_args.thumbnail_size}[/cyan]"
        )
        ...
        if num_heic > 0:
            ...
            batch_convert_heic(
                heic_handler.heic_files, output_dir=input_dir, move_originals=True
            )
        ...
        ui.print_info(
            f"[cyan]Found {num_images} JPG file{'s' if num_images != 1 else ''}[/cyan]\n"
        )
        ...
        logger.info(
            f"Processing complete - Processed: {stats['processed']}, Skipped: {stats['skipped_no_gps']}, Errors: {stats['errors']}"
        )
        ...
        if not processed_images:
            ui.print_summary(
                ...
                None,
                None,
                processor.get_no_gps(),
                processor.get_no_direction(),
            )
            return 0
        ...
        try:
            ...
            if max_images <= 0:
                ...
                chunks = [
                    processed_images[i : i + max_images]
                    for i in range(0, len(processed_images), max_images)
                ]
            ...
            try:
                ...
                for chunk_index, chunk in enumerate(chunks):
                    ...
                    if len(chunks) == 1:
                        ...
                        current_output_path = get_absolute_path(
                            base_path.with_name(
                                f"{base_path.stem}_{chunk_index + 1}{base_path.suffix}"
                            )
                        )
                    ...
                    try:
                        for img_data in chunk:
                            ...
                            ui.update_progress(
                                global_index, len(processed_images), filename
                            )
